...
class YOLODetector(AbstractDetector):
    ...
    def __init__(self):
        ...
        # some constants that are used across this class
        self.LABELS = ['tank']
        ...
        #############################################################################################
        self.model = None
        self.dummy_array = None

    # the function to implement the orgnization layer
    def space_to_depth_x2(self, x):
        return tf.space_to_depth(x, block_size=2)

    # builds the initial graph for inference
    def build_graph(self):
        '''
        This function builds the model graph
        '''
    ...
    def load_weights(self, path):
        ...
        self.dummy_array = np.zeros((1,1,1,1,self.TRUE_BOX_BUFFER,4))
        return None

    # detects class objects from the given frame
    def detect(self, frame):
        """detects the concerned object class in the given frame"""
        ...
        boxes = decode_netout(netout[0], obj_threshold=0.3,
                              nms_threshold=self.NMS_THRESHOLD,
                              anchors=self.ANCHORS, nb_class=self.CLASS)
        print("number of boxes: ", len(boxes)) # DEBUGGING PRINT
        final_image = draw_boxes(frame, boxes, self.LABELS, (0,255,0))
        rectangles = [self.convert_yolo2cv(box, frame) for box in boxes]
        return rectangles, final_image

    # converts boxes from YOLO representation to standard OpenCV representation
    # for drawing
    def convert_yolo2cv(self, box, image):
        """Converts yolo rectangle representation into OpenCV representation
        box: center(x,y) and dimensions (w, h)
